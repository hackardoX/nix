...
install -d -m755 "$temp/run/secrets"
...
op read --no-newline "op://Development/Hetzner HomeLab/hashed user password" >"$temp/run/secrets/.hetzner_password"

# Set the correct permissions
chmod 600 "$temp/etc/opnix-token"
chmod 600 "$temp/etc/ssh/ssh_host_ed25519_key"
chmod 644 "$temp/etc/ssh/ssh_host_ed25519_key.pub"
chmod 600 "$temp/run/secrets/.hetzner_password"
